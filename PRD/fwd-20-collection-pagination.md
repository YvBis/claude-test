...
**Поправка 2026-09-26 (fwd-25):** инвентарь выше неполон в одном существенном месте. `?limit[]=1` давал
400 не благодаря `ctype_digit` внутри `parsePagination`, а благодаря самому `InputBag::get()`, который
бросает `BadRequestException` раньше любого кода приложения — ответом при этом была фреймворковая
HTML-страница, а не конверт API. fwd-25 перевёл `parsePagination` на чтение сырого массива
`$request->query->all()` (подробности — `PRD/fwd-25-non-scalar-query-params.md`), и теперь 400 за
`?limit[]`/`?offset[]` приходит нашим конвертом, как и положено.

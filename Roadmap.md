...
Девять пунктов ниже (`fwd-17`…`fwd-25`) — один связный мини-этап, а не разрозненный долг; PRD: `PRD/mini-stage-5A-error-contract.md`. Порядок работ: `fwd-20` → `fwd-25` → `fwd-19` → `fwd-17` → `fwd-21` → `fwd-22` → `fwd-24` → `fwd-23` → `fwd-18` (`fwd-17`/`fwd-19`/`fwd-21` делят один `kernel.exception`-listener). Четыре пункта помечены `[bug]` — это не долг, а дефекты контракта.
...
| fwd-25 | [bug] Массив в `owner` молча игнорируется: `GET /api/collections?owner[]=x` не проходит проверку `is_string` в `CollectionController::list`, управление падает в ветку `else`, и клиент получает **свои** коллекции вместо запрошенных чужих — фильтр тихо отброшен, статус 200. Отклонять не-строковый `owner` через `400` (симметрично `limit`/`offset`, где `?limit[]=1` уже даёт 400 благодаря `ctype_digit`). Тот же класс есть у `ItemController::name[]` и `TagController::search[]` (фильтр тихо исчезает) — единая политика в решении 8 мини-этапа 5A. Тесты: `?owner[]=x` → 400, комбинация `?owner[]=x&limit=abc` (приоритет пагинации), `?owner[]=x&owner=y`. **Влияние:** массив или повторяющийся параметр вместо строки возвращает правдоподобный, но неверный результат. Estimate: 0.5ч | todo | From 2026-09-23: senior-review fwd-20 — `?owner[]=x` падает в ветку «свои коллекции» |

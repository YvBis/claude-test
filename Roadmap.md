...
| fwd-3 | [future] Единый контракт сериализации response-DTO: интерфейс `ArrayableInterface` (`toArray(): array`) в `src/Application/Common/DTO/`; реализовать на `CollectionDTO`, `ItemDTO`, `TagDTO` и будущих response-DTO (4.5, 5.x). Сейчас `toArray()` дублируется в трёх DTO без общего контракта; потребителей-дженериков нет, поэтому чистое будущее-улучшение. Estimate: 0.5-1ч | todo | From 2026-09-12: предложено пользователем при обсуждении ItemDTO (Task 4.4) |

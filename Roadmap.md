...
| 5.13 | [review] Перевести `symfony-lsp` в CI на runtime-режим (маршруты, DI-контейнер, Doctrine-метаданные): добавить MySQL/Redis-сервисы и генерацию JWT-ключей (образец — job `unit-tests` + `setup-ci`); после недели зелёного пилота сделать job блокирующим. Локально проверено: runtime `exit 0`, `complete: true`. Дёшевые альтернативы (architect-review 5.12): `bin/console debug:container` при компиляции контейнера + Symfony-лог депрекейшенов (`deprecation_messages`) — ловят конфиг-депрекейшены без runtime-прогона; сравнить перед промоушеном. **Надёжность скачивания** (наблюдение PR #74): `curl` дважды упал на транзиентном `504` при загрузке релизного ассета — добавить retry с backoff и кэш бинарника (`actions/cache` по версии + SHA256), чтобы флейк CDN не ронял job. | todo |
| 5.14 | [review] Сделать CI-job `symfony-diagnostics` блокирующим: убрать `continue-on-error`, добавить в `needs` у `ci-summary` — решение по итогам недели пилота (~2026-09-27, не дожидаясь 5.13). Заодно: закрепить ожидаемый SHA256 бинарника константой в репо (сейчас `SHA256SUMS` тянется из того же релиза — TOFU-доверие) и завести регулярную проверку версии `symfony-lsp` (0.x-релизы частые, Dependabot скачиваемый бинарник не видит). **Code Review: потерянные вердикты** (наблюдение PR #74): job `OpenRabbit Review` несколько раз завершился `CANCELLED` — concurrency-группа `ai-review-${{ github.ref }}` с `cancel-in-progress: true` (`.github/workflows/code-review.yml:11-13`) отменяет прогон при каждом новом push в head (для «не ревьюить устаревший head» это ожидаемо, но вердикт теряется), а `timeout-minutes: 15` (`:20`) при медленном free-tier LLM способен добить прогон. Решить: поднять timeout и/или сделать группу неотменяемой, и явно трактовать `cancelled` как «вердикта нет», а не как пройденное ревью. | todo |
| 5.15 | [review] Выровнять минорные версии Symfony: `symfony/redis-messenger` 7.3.10 → 7.4.x (остальное уже на 7.4.x) + батч patch-бампов по списку `composer outdated --direct` от 2026-09-20; после — `composer ci:all`. Мажоры (phpunit 12, rector 2.6) не трогать. | todo |

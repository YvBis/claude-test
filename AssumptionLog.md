...
**Почему отмена не детектируется здесь.** Шаг внутри job'а не может наблюдать отмену или убийство по `timeout-minutes` своего же job'а — последующие шаги не выполняются. Это свойство runner'а, поэтому задача переформулирована: детекция `cancelled`/`timed_out` возможна только снаружи через `workflow_run` (часть 5.19B), а здесь закрыто только достижимое изнутри.
